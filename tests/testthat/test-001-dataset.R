test_that("Create training dataset", {
  ...
  expect_equal(dim(dataset$.getitem(1)$fields), c(dataset$fields_max, 7))
...
test_that("Create validation/test dataset", {
  ...
  expect_equal(dim(dataset.valid$.getitem(1)$fields), c(fields_max, 7))
  ...
  expect_equal(dim(batch$fields), c(farms_count, fields_max, 7))
  expect_equal(dim(batch$fertilizers), c(farms_count, nrow(apus::fertilizers), 2))
})
